...
void IntroState::Init(GameEngine* game)
{
	...
	for (int i = 0; i < MAX_MENU; i++)
	{
		...
		m_MenuTransform[i] = (Transform*)menu->FindComponent(COMPONENT_NAME[TRANSFORM]);
		...
		m_MenuTransformText[i] = (Transform*)menuText->FindComponent(COMPONENT_NAME[TRANSFORM]);
		m_MenuTransformText[i]->m_fX = MENU_TEXT_X[i];
		m_MenuTransformText[i]->m_fY = MENU_TEXT_Y[i];
		m_Layer[2]->Add(menuText);
		if (i == SOUND_TYPE)
		{			
			m_MenuSoundText = (Text*)menuText->FindComponent(COMPONENT_NAME[TEXTCOMP]);
	...
	m_SelectBarTramsform = (Transform*)selectBar->FindComponent(COMPONENT_NAME[TRANSFORM]);
	...
	Transform* titleTrans = (Transform*)title->FindComponent(COMPONENT_NAME[TRANSFORM]);
	...
	Composite* highScoreComp = Composite::CreateText(game, "score", strScore);
	Transform* highScoreTrans = (Transform*)highScoreComp->FindComponent(COMPONENT_NAME[TRANSFORM]);
	highScoreTrans->m_fX = 4;
	highScoreTrans->m_fY = 4;
	m_Layer[2]->Add(highScoreComp);


	Composite* onePlayer = Composite::CreateSprite(game, "1P", "1P.bmp");
	Transform* onePlayerTrans = (Transform*)onePlayer->FindComponent(COMPONENT_NAME[TRANSFORM]);
	...
	Transform* twoPlayerTrans = (Transform*)twoPlayer->FindComponent(COMPONENT_NAME[TRANSFORM]);
	...
	m_ClickSound = (Sound*)soundComp->FindComponent(COMPONENT_NAME[SOUND]);
...
void IntroState::HandleEvents(GameEngine* game)
{
	...
	{
		m_nMenuIndex++;
		if (m_nMenuIndex >= MAX_MENU)
		{
			m_nMenuIndex = MAX_MENU - 1;
		}
		m_SelectBarTramsform->m_fX = MENU_SLECT_X[m_nMenuIndex];
		m_SelectBarTramsform->m_fY = MENU_SLECT_Y[m_nMenuIndex];
		m_ClickSound->Play();
	...
	{
		RegDataManager::GetInstance()->DeleteValue("Score");
	}
