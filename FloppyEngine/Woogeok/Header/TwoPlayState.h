...
class TwoPlayState : public GameState
{
	...
	//두번째 플레이어
	Composite* m_TwoPlayerComp;
	Transform* m_TwoPlayerTrans;
	...
	//피격당하면 몃초 동안 무적 상태이다.
	bool m_bTwoInvincible = false;
	float m_fTwoPlayerInvincibleTimeElapsed = 0.0f;
...
public:
	...
	void TwoPlayerInit(GameEngine* game);
	...
	void FruitUpdate();
	void CarUpdate();
	void CollisionUpdate(GameEngine* game);
	void TwoCollisionUpdate(GameEngine* game);
	void FruitEffectStart(int nFruitIndex);
	void PalyerSpeedUpUpdate();
	void PalyerInvincibleUpdate();
	void TwoPalyerSpeedUpUpdate();
	void TwoPalyerInvincibleUpdate();
